...
2. 다음으로 commutative ring의 category를 $$\mathtt{CRINGS}$$이라고 하자. 고정된 자연수 $$n$$에 대하여, 다음과 같은 functor를 정의하자.:
   ...
   $$\det$$는 선형대수에서 했듯 주어진 commutative ring $$A$$에 대해 matrix $$K=(k*{ij})\in M_n(A)$$가 주어지면 $$\det_A(K):=\sum_{\sigma\in S_n}\text{sgn}(\sigma)\prod_{i=1}^n k_{\sigma(i)i}$$로 정의되는 함수다. 이제 다음의 diagram이 commutative인지 확인해보도록 하겠다.:
   ...
   $$
      \begin{aligned}
        f(\det_A((k_{ij}))) &= f(\sum_{\sigma\in S_n}\text{sgn}(\sigma)\prod_{i=1}^n k_{\sigma(i)i}) \\
                            &= \sum_{\sigma\in S_n}\text{sgn}(\sigma)\prod_{i=1}^n f(k_{\sigma(i)i}) \\
                            &= \det_B((f(k_{ij})))
      \end{aligned}
   $$
