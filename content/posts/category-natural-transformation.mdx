...
$$F,G:\mathcal{C}\longrightarrow\mathcal{D}$$가 functors라고 하자. 이 때 $$F$$와 $$G$$ 사이의 `morphism`(or `natural transformation`) $$\alpha:F\longrightarrow G$$는 아래의 조건을 만족시키는 $$\mathcal{D}$$의 morphism collection $${\alpha*X:::FX\longrightarrow GX}*{X\in\mathcal{C}}$$이다.:
